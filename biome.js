...
export function generateBiomes() {
  for(let x=0;x<W;x++){
    if(x<W/4) biomeMap[x]="mountain";
    else if(x<W/2) biomeMap[x]="ocean";
    else if(x<W*3/4) biomeMap[x]="desert";
    else biomeMap[x]="plains";
  }
}
  ...
  for(let y=0;y<H;y++){
    ...
    for(let x=0;x<W;x++){
      ...
      if(biome==="mountain"){
        ...
        else if(y>=40){ world[y][x]=7; worldHP[y][x]=2; } // water
        else { world[y][x]=0; worldHP[y][x]=0; }
